...
def make_titlecard(uid):
    ...
    proc = subprocess.run(
        [
            ...
            "-f", "lavfi", "-i", "anullsrc=channel_layout=stereo:sample_rate=48000", "-loop", "1", "-i", titlecard_path, "-t", "10", "-shortest",
            *ffmpeg_encoder_args,
            output_path
        ]
    )
...
def make_session_video(session_uid, skip_missing, overwrite):
    ...
    if skip_missing:
        uid_list = [uid for uid, pos in get_session_schedule(session_uid) if has_media_file(uid)]
    else:
        uid_list = [uid for uid, pos in get_session_schedule(session_uid)]
